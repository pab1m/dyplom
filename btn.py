...
btn_stats_now = KeyboardButton(text="1️⃣ Статистика за цей акт")
...
btn_controller = KeyboardButton(text="💨 СПЕЦІАЛІСТ")
btn_duelist = KeyboardButton(text="⚔ ДУЕЛЯНТ")
btn3_sentinel = KeyboardButton(text="📸 ВАРТОВИЙ")
btn_initiator = KeyboardButton(text="🎯 ІНІЦІАТОР")
...
controller = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
controller.add(btn_brim, btn_viper).row(btn_omen, btn_astra, btn_harbor).add(btn_agents)
...
maps.add(btn_lotus, btn_pearl, btn_fracture, btn_icebox, btn_breeze, btn_bind, btn_ascent, btn_split, btn_haven).add(btn_for_new)
